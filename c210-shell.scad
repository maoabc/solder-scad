...
module c210_pcb(){
    translate([pcb_h/2,pcb_t/2,0])
        ...
        union(){
            ...
            translate([38,pcb_h/2,pcb_t])
                key();
            translate([45,pcb_h/2,pcb_t])
                key();
            //模拟突起的电容
...
module c210_top_panel(){
    ...
    difference(){
        ...
        translate([-(handle_inner_width)/2,0,pcb_w-30])
            cube([handle_inner_width, handle_inner_thickness,30]);
            translate([lcd_h/2,-(handle_inner_thickness/2-lcd_t+0.4)-0.001,lcd_off])
